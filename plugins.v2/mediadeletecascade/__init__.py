...
class MediaDeleteCascade(_PluginBase):
    ...
    def get_page(self) -> List[dict]:
        ...
        summary = {
            "total": len(history),
            "done": sum(1 for item in history if item.get("status") == "完成"),
            "skipped": sum(1 for item in history if item.get("status") == "跳过"),
            "errors": sum(1 for item in history if item.get("status") == "失败"),
            "deleted_files": sum(len(item.get("deleted_files") or []) for item in history),
        }
        contents: List[dict] = [
            {
                "component": "VAlert",
                "props": {
                    ...
                    "variant": "tonal",
                    "text": (
                        f"共记录 {summary['total']} 次同步删除；完成 {summary['done']} 次，"
                        f"跳过 {summary['skipped']} 次，失败 {summary['errors']} 次，"
                        f"同步删除硬链接文件 {summary['deleted_files']} 个。"
                    ),
                },
            },
            {
        ...
        for item in history[:80]:
            ...
            color = "success" if status == "完成" else "warning" if status == "跳过" else "error"
            details = [
                f"触发路径：{item.get('trigger_path')}",
                f"处理方式：{item.get('mode')}",
            ]
            if item.get("reason"):
                details.append(f"原因：{item.get('reason')}")
            if item.get("deleted_files"):
                details.extend([f"删除硬链接：{path}" for path in item.get("deleted_files")[:8]])
                if len(item.get("deleted_files")) > 8:
                    details.append(f"还有 {len(item.get('deleted_files')) - 8} 个硬链接文件未展开显示")
            if item.get("actions"):
                details.append("联动操作：" + "、".join(item.get("actions")))
            if item.get("error"):
                details.append(f"错误：{item.get('error')}")
                            ...
                            "content": [
                                {
                                    ...
                                    "content": [
                                        ...
                                        },
                                        {
                                            "component": "VChip",
                                            "props": {"variant": "outlined", "size": "small"},
                                            "text": f"硬链接 {len(item.get('deleted_files') or [])}",
                                        },
                                    ],
                                },
                                {
        ...
        contents.append({"component": "div", "content": cards})
        return contents
    ...
    def _append_sync_history(
        ...
        actions = []
        if self._delete_torrents and not self._is_scrap_file(trigger_path):
            actions.append("联动下载任务/种子")
        if self._delete_history:
            actions.append("清理转移记录")
        if self._delete_scrap_infos:
            actions.append("清理刮削文件")
        if self._delayed_deletion and mode == "延迟删除":
            actions.append(f"延迟 {self._delay_seconds} 秒确认")

        record = {
            "time": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),
            "status": status,
            "mode": mode,
            "title": trigger_path.name,
        ...
        self.save_data("history", history[: self._history_limit])
    ...
    def _execute_delayed_deletion(self, task: DeletionTask):
        ...
        try:
            ...
            self._append_sync_history(
                ...
                reason="未找到可同步删除的硬链接文件" if not deleted_files else "",
            )
